...
def process(image):
    ...
    lines= cv2.HoughLinesP(cropped_img,rho=6,theta=np.pi/60,threshold=100,lines=np.array([]),minLineLength=100,maxLineGap=20)

    image_with_lines =draw_the_lines(image,lines)

    return image_with_lines

cap = cv2.VideoCapture(r"C:\Users\User\Desktop\Artificial Intelligence\road.mp4")
